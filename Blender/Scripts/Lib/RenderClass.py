...
class RenderCore:
    def __init__(self, config):
        ...
        path = os.path.dirname(path)
        if self.config.material == "wireframe":
            if self.config.wireframe_mode == 1:
                self.config.texture_path = path + "/Data/Texture/checkerboard.png"
            ...
            if self.config.wireframe_mode == 4:
                ...
                self.config.wireframe_color = (0.4, 0.2, 0, 1.0)
