...
def setup_renderer(output_path, width, height):
...
def build_singularity_primitives():
    scene_collection = bpy.context.scene.collection
    for index, color in singular_colors.items():
        ...
        color_node.outputs[0].default_value = hex2rgba(color)
...
def build_addons(scene_json):
    ...
    for i, s in enumerate(scene_json['singularities']):
        ...
        bpy.ops.object.collection_instance_add(
            collection=primitive_name, location=blender_vec(s['position']))
    ...
    for i, c in enumerate(scene_json['cuts']):
        p0 = mathutils.Vector(blender_vec(c['points'][0]))
        p1 = mathutils.Vector(blender_vec(c['points'][1]))
...
def build_camera(scene_json):
    cam_json = scene_json['camera']
    cam = bpy.data.cameras['Camera']
    cam.lens_unit = 'FOV'
    cam.lens = 20
    cam_obj = bpy.data.objects.new('Camera', cam)
    cam_obj.location = blender_vec(cam_json['position'])
...
def build_indirect_light():
    ...
    envmap_node.image = bpy.data.images.load(envmap_path)
